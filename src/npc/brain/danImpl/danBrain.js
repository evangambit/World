 ...
 *
 * Currently implements exploration only.
 */
...
export class DanBrain {
     ...
     */
    tick(_world, _dt, _gameTime, _actionProgress, _visibleTiles, lastActionResult = null) {
        const npc = this._npc;
     ...
     * best exploration goal. Returns a walkToLocation generator aimed at that
     * goal, or null if there is nowhere left to explore.
     *
    ...
    _chooseTask() {
        ...
        return walkToLocation(npc, hypoWorld, goal, {
            // Rebuild a fresh HypotheticalWorld on each replan so newly
            // perceived tiles are reflected in path checks.
            getWorld: () => {
                const mem = getNpcTileMemoryStore(npc);
                return mem ? createHypotheticalFromMemory(mem) : hypoWorld;
            },
        });
    }
    ...
    getStatus() {
        if (!this._currentGoal) return { lines: ['idle'] };
